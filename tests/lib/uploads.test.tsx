...
vi.mock('@/lib/idb', async () => {
  ...
  return {
    ...
    saveDoc: vi.fn().mockResolvedValue({
      id: 'k:doc:1',
      jobKey: 'k',
      title: 'T',
      createdAt: Date.now(),
...
describe('UploadsPanel', () => {
  ...
  it('creates a document record', async () => {
    ...
    fireEvent.change(screen.getAllByPlaceholderText('Title (e.g., Contract, Progress Report)')[0], {
      target: { value: 'My Doc' },
    });
    fireEvent.change(screen.getAllByPlaceholderText('Notes / Content (quick text)')[0], {
      target: { value: 'Hi' },
    });
