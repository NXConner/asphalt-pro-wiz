  ...
  paths: {
    '/api/estimates': {
      post: {
        ...
        description: 'Calculates a project estimate based on input parameters (area, materials, services).',
        tags: ['Estimates'],
        requestBody: {
          required: true,
    ...
    '/api/ai/chat': {
      post: {
        ...
        description: 'Proxy to Gemini chat through Supabase edge function, augmented with repo RAG context.',
        tags: ['AI'],
        requestBody: {
          required: true,
          content: { 'application/json': { schema: { type: 'object', properties: { question: { type: 'string' } }, required: ['question'] } } },
        },
        responses: {
    ...
    '/api/ai/image': {
      post: {
        ...
        responses: {
          '200': { description: 'Analysis response', content: { 'application/json': { schema: { type: 'object', properties: { text: { type: 'string' } } } } } },
        },
      },
    },
  },
