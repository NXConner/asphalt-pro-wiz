...
export function WeatherAdvisor({ coords }: { coords: [number, number] | null }) {
  ...
  useEffect(() => {
    ...
    const apiKey = settings.openWeatherApiKey || (import.meta as any)?.env?.VITE_OPENWEATHER_API_KEY;
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div>
          ...
          <Slider value={[popThreshold]} min={0} max={100} step={5} onValueChange={(v) => setPopThreshold(v?.[0] ?? 30)} />
        </div>
        {windows.length === 0 ? (
          <div className="text-sm text-muted-foreground">Set OpenWeather API key in Map settings and choose a location to see suggested windows.</div>
        ) : (
            ...
            {windows.map((b) => (
              <div key={b.dt} className="border rounded p-2 text-sm">
                ...
                <div className="text-muted-foreground">
                  POP {(Math.round((b.pop ?? 0) * 100))}% • {Math.round(b.main?.temp ?? 0)}°F • {b.weather?.[0]?.description || "—"}
                </div>
              </div>
            ))}
          </div>
