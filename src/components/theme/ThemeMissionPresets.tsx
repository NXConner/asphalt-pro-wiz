...
export interface ThemeMissionPresetsProps {
  groups: ThemePresetGroup[];
  activeTheme: ThemeName;
  mode: ThemeMode;
  onModeChange: (value: ThemeMode) => void;
  onSelectPreset: (presetId: ThemeName) => void;
}
...
interface ThemeGroupProps {
  ...
  activeTheme: ThemeName;
  onSelect: (presetId: ThemeName) => void;
}

function ThemeGroup({ group, activeTheme, onSelect }: ThemeGroupProps) {
  const { label, description, presets, category } = group;
  return (
    <div className="space-y-3 rounded-xl border border-white/10 bg-slate-950/70 p-4">
      <div>
        ...
        <p className="mt-1 text-xs text-slate-300/60">{description}</p>
      </div>
