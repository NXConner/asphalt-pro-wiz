...
export function CustomizableCard({
  ...
  const boxShadow = (() => {
    ...
    switch (level) {
      case 1: return '0 4px 10px rgba(0,0,0,0.08)';
      case 2: return '0 6px 16px rgba(0,0,0,0.12)';
      case 3: return '0 10px 24px rgba(0,0,0,0.16)';
      case 4: return '0 14px 30px rgba(0,0,0,0.22)';
      case 5: return '0 18px 40px rgba(0,0,0,0.28)';
  ...
  const animationClass = (() => {
    switch (style.animation) {
      case 'float': return 'pps-anim-float';
      case 'glow': return 'pps-anim-glow';
      case 'shimmer': return 'pps-anim-shimmer';
      case 'pulse': return 'pps-anim-pulse';
      default: return undefined;
    }
  })();
  ...
  const variantClass = (() => {
    switch (style.layoutVariant) {
      case 'compact': return 'pps-variant-compact';
      case 'dense': return 'pps-variant-dense';
      case 'padded': return 'pps-variant-spacious';
      case 'default':
      default: return undefined;
    }
  })();
  ...
  return (
    <Card
      className={cn('relative transition-all duration-300', style.hoverLift && 'pps-hover-lift', animationClass, variantClass, className)}
      style={cardStyle}
    >
          ...
          <PopoverContent className="w-96" align="end">
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={style.backgroundType || 'solid'} onValueChange={(v) => handleStyleChange('backgroundType', v as CardStyle['backgroundType'])}>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="grid grid-cols-6 gap-2">
                      {['hsl(var(--card))', 'hsl(var(--primary) / 0.08)', 'hsl(var(--secondary) / 0.18)', 'hsl(var(--accent) / 0.18)', 'transparent', 'hsl(var(--background) / 0.6)'].map((color) => (
                        <button
                          key={color}
                          className="h-8 rounded border"
                          style={{ background: color, outline: style.backgroundColor === color ? '2px solid hsl(var(--primary))' : 'none' }}
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 items-center">
                    <div className="flex items-center gap-2">
                      ...
                      <input type="color" value={style.gradientFrom || '#1f2937'} onChange={(e) => handleStyleChange('gradientFrom', e.target.value)} className="h-8 w-10 border rounded" />
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-muted-foreground">To</span>
                      <input type="color" value={style.gradientTo || '#0ea5e9'} onChange={(e) => handleStyleChange('gradientTo', e.target.value)} className="h-8 w-10 border rounded" />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <Label className="text-xs">Angle: {style.gradientAngle ?? 135}°</Label>
                    <Slider value={[style.gradientAngle ?? 135]} onValueChange={([v]) => handleStyleChange('gradientAngle', v)} min={0} max={360} step={5} />
                  </div>
                </div>
              )}
              ...
              {style.backgroundType === 'image' && (
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <input type="file" accept="image/*" onChange={handleImageSelect} aria-label="Upload background image" />
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={style.backgroundSize || 'cover'} onValueChange={(v) => handleStyleChange('backgroundSize', v as 'cover' | 'contain' | 'auto')}>
                      <SelectTrigger className="h-8">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
              ...
              <div className="space-y-2">
                ...
                <Slider value={[style.borderRadius || 0]} onValueChange={([v]) => handleStyleChange('borderRadius', v)} max={24} step={1} />
              </div>

              <div className="space-y-2">
                <Label className="text-xs">Shadow Level: {style.shadowLevel ?? 0}</Label>
                <Slider value={[style.shadowLevel ?? 0]} onValueChange={([v]) => handleStyleChange('shadowLevel', v)} max={5} step={1} />
              </div>

              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-2">
                  <Label className="text-xs">Animation</Label>
                  <Select value={style.animation || 'none'} onValueChange={(v) => handleStyleChange('animation', v as CardStyle['animation'])}>
                    <SelectTrigger className="h-8">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  {onResizePreset && (
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <Button size="sm" variant="secondary" onClick={() => onResizePreset(4, 6)}>Small</Button>
                        <Button size="sm" variant="secondary" onClick={() => onResizePreset(6, 8)}>Medium</Button>
                        <Button size="sm" variant="secondary" onClick={() => onResizePreset(8, 12)}>Large</Button>
                        <Button size="sm" variant="secondary" onClick={() => onResizePreset(4, 12)}>Tall</Button>
                        <Button size="sm" variant="secondary" onClick={() => onResizePreset(10, 6)}>Wide</Button>
                      </div>
                    </div>
                  )}
                  {onLayoutFlagsChange && (
                    <div className="grid grid-cols-3 gap-3">
                      <div className="flex items-center justify-between">
                        ...
                        <Switch onCheckedChange={(checked) => onLayoutFlagsChange({ isDraggable: checked })} />
                      </div>
                      <div className="flex items-center justify-between">
                        <Label className="text-xs">Resizable</Label>
                        <Switch onCheckedChange={(checked) => onLayoutFlagsChange({ isResizable: checked })} />
                      </div>
                      <div className="flex items-center justify-between">
                        <Label className="text-xs">Lock</Label>
                        <Switch onCheckedChange={(checked) => onLayoutFlagsChange({ static: checked })} />
                      </div>
                    </div>
                  )}
                </div>
