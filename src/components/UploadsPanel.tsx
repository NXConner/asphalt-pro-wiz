  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Upload images and files, and create/edit internal documents.</CardDescription>
      </CardHeader>
            ...
            {files.map((f) => (
              <div key={f.id} className="p-3 border rounded-md flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-sm font-medium truncate max-w-[200px]" title={f.name}>{f.name}</p>
                    <p className="text-xs text-muted-foreground">{(f.size/1024).toFixed(1)} KB • {new Date(f.createdAt).toLocaleString()}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <a
                  ...
                  <Button variant="ghost" size="icon" onClick={() => handleDeleteFile(f.id)} className="text-destructive">
            ...
            {docs.map((d) => (
              <div key={d.id} className="p-3 border rounded-md flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="ghost" size="icon" onClick={() => handleDeleteDoc(d.id)} className="text-destructive">
                    <Trash2 className="w-4 h-4" />
                  </Button>
                </div>
              </div>
