...
export async function fetchRadarFrames(): Promise<RadarFrame[]> {
  ...
  try {
    ...
    if (resp.ok) {
      ...
      const frames: RadarFrame[] = json.timestamps.map((t) => ({
        time: t,
        path: `${json.host}/v2/radar/${t}/256/{z}/{x}/{y}/2/1_1.png`,
      }));
  ...
  try {
    ...
    if (resp2.ok) {
      ...
      const frames: RadarFrame[] = [...(json?.radar?.past || []), ...(json?.radar?.nowcast || [])]
        ...
        .map((f: any) => ({
          time: f.time,
          path: `https://tilecache.rainviewer.com${f.path}/256/{z}/{x}/{y}/2/1_1.png`,
        }));
      saveCached(CACHE_KEY, frames);
      return frames;
    }
