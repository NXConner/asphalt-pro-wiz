      ...
      <main className="mx-auto flex min-h-screen max-w-5xl flex-col gap-6 px-4 py-16 text-slate-100">
        ...
        <Card className="bg-white/5">
          <CardContent className="space-y-3 py-6 text-slate-200">
            <p>This feature is behind the <code>commandCenter</code> flag.</p>
            <p>
              Enable it in the settings panel or by setting <code>VITE_FLAG_COMMANDCENTER=1</code> in your
              environment.
            </p>
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-6xl flex-col gap-8 px-4 py-12 text-slate-100">
      <header className="space-y-3">
        <p className="text-xs uppercase tracking-[0.3em] text-slate-200/70">Executive Command Center</p>
        <h1 className="text-4xl font-semibold">Health Snapshot</h1>
        <p className="max-w-3xl text-sm text-slate-200/80">
          High-level analytics distilled from Supabase data. Use these metrics to steer scheduling, monitor
          revenue, and identify which missions demand attention.
        </p>
      </header>

      <section className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <MetricCard title="Active Jobs" value={metrics.totals.activeJobs} footer="Need estimate / scheduled" />
        ...
        <Card className="bg-white/5">
          ...
          <CardContent className="space-y-3 text-sm text-slate-200/80">
            ...
            <p className="text-xs text-slate-400">
              Turnaround measures the median days between job creation and the first estimate. Crew coverage tracks
              how many jobs have an upcoming assignment.
            </p>
          </CardContent>
        </Card>
      </section>
  ...
  return (
    <Card className={background}>
      ...
      <CardContent className="space-y-2">
        ...
        {footer ? <p className="text-xs uppercase tracking-[0.3em] text-slate-300/70">{footer}</p> : null}
      </CardContent>
    </Card>
  );
}
